...
class Interpreter {
 ...
 private:
  ...
  void interpretIdentifierOperation(const std::string& identifier, CallStackFrame& frame) {
    ...
    if (current(frame).getType() == TokenType::LAMBDA) {
      ...
      throw InvalidOperationError(current(frame), "Invalid operation near `" + identifier + "`.");
    }
  }
