...
class Method {
 public:
  ...
  void addParameterValue(std::string paramName, std::variant<int, double, bool, std::string> value) {
    parameterKVP[paramName] = value;
  }
  bool hasParameter(std::string& paramName) {
    return parameterKVP.find(paramName) != parameterKVP.end(); 
  }
  std::variant<int, double, bool, std::string> getParameterValue(std::string& paramName) {
 ...
 private:
  ...
  std::string _name;
  std::map<std::string, std::variant<int, double, bool, std::string>> parameterKVP;
};
