namespace kiwi.Parsing;

public enum TokenName
{
  Default,
  Arrow,
  Builtin_Callable_Call,
  Builtin_Callable_Parameters,
  Builtin_Callable_Returns,
  Builtin_Callable_ToLambda,
  Builtin_Channel_Close,
  Builtin_Channel_Create,
  Builtin_Channel_Recv,
  Builtin_Channel_Send,
  Builtin_Channel_TryRecv,
  Builtin_Compression_CompressBrotli,
  Builtin_Compression_CompressDeflate,
  Builtin_Compression_CompressGzip,
  Builtin_Compression_CompressZstd,
  Builtin_Compression_DecompressBrotli,
  Builtin_Compression_DecompressDeflate,
  Builtin_Compression_DecompressGzip,
  Builtin_Compression_DecompressZstd,
  Builtin_Compression_ZipCreate,
  Builtin_Compression_ZipExtract,
  Builtin_Csv_Parse,
  Builtin_Csv_ParseFile,
  Builtin_Csv_ParseMaps,
  Builtin_Csv_ParseFileMaps,
  Builtin_Sqlite_Open,
  Builtin_Sqlite_Close,
  Builtin_Sqlite_Exec,
  Builtin_Sqlite_ExecParams,
  Builtin_Sqlite_Query,
  Builtin_Sqlite_QueryParams,
  Builtin_Sqlite_LastInsertId,
  Builtin_Sqlite_Changes,
  Builtin_Json_Parse,
  Builtin_Json_Stringify,
  Builtin_Json_ParseMaps,
  Builtin_Protobuf_Encode,
  Builtin_Protobuf_Decode,
  Builtin_Xml_Parse,
  Builtin_Xml_ParseFile,
  Builtin_Xml_ParseMaps,
  Builtin_Xml_ParseFileMaps,
  Builtin_Console_Background,
  Builtin_Console_Clear,
  Builtin_Console_CursorVisible,
  Builtin_Console_Foreground,
  Builtin_Console_ReadKey,
  Builtin_Console_ReadLine,
  Builtin_Console_Reset,
  Builtin_Console_Title,
  Builtin_Console_WindowSize,
  Builtin_Core_Append,
  Builtin_Core_BeginsWith,
  Builtin_Core_Between,
  Builtin_Core_Chars,
  Builtin_Core_Chomp,
  Builtin_Core_Clear,
  Builtin_Core_Clone,
  Builtin_Core_Concat,
  Builtin_Core_Contains,
  Builtin_Core_Count,
  Builtin_Core_Day,
  Builtin_Core_Dequeue,
  Builtin_Core_Empty,
  Builtin_Core_EndsWith,
  Builtin_Core_Enqueue,
  Builtin_Core_Find,
  Builtin_Core_First,
  Builtin_Core_Flatten,
  Builtin_Core_Get,
  Builtin_Core_HasKey,
  Builtin_Core_HexBytes,
  Builtin_Core_Hour,
  Builtin_Core_IndexOf,
  Builtin_Core_Insert,
  Builtin_Core_IsA,
  Builtin_Core_Join,
  Builtin_Core_Keys,
  Builtin_Core_Last,
  Builtin_Core_LastIndexOf,
  Builtin_Core_LeftTrim,
  Builtin_Core_Lines,
  Builtin_Core_Lowercase,
  Builtin_Core_Match,
  Builtin_Core_Matches,
  Builtin_Core_MatchesAll,
  Builtin_Core_NamedCaptures,
  Builtin_Core_RegexEscape,
  Builtin_Core_ScanGroups,
  Builtin_Core_Merge,
  Builtin_Core_Millisecond,
  Builtin_Core_Minute,
  Builtin_Core_Month,
  Builtin_Core_Ord,
  Builtin_Core_Pop,
  Builtin_Core_Pretty,
  Builtin_Core_Push,
  Builtin_Core_Remove,
  Builtin_Core_RemoveAt,
  Builtin_Core_Replace,
  Builtin_Core_Reverse,
  Builtin_Core_RightTrim,
  Builtin_Core_Rotate,
  Builtin_Core_RReplace,
  Builtin_Core_RSplit,
  Builtin_Core_Scan,
  Builtin_Core_Second,
  Builtin_Core_Set,
  Builtin_Core_Shift,
  Builtin_Core_Size,
  Builtin_Core_Slice,
  Builtin_Core_Split,
  Builtin_Core_Substring,
  Builtin_Core_Swap,
  Builtin_Core_ToBytes,
  Builtin_Core_ToDate,
  Builtin_Core_ToFloat,
  Builtin_Core_ToHex,
  Builtin_Core_ToInteger,
  Builtin_Core_ToList,
  Builtin_Core_ToString,
  Builtin_Core_Trim,
  Builtin_Core_Truthy,
  Builtin_Core_Type,
  Builtin_Core_Unique,
  Builtin_Core_Unshift,
  Builtin_Core_Uppercase,
  Builtin_Core_Values,
  Builtin_Core_Year,
  Builtin_Core_Zip,
  Builtin_Encoder_Base64Decode,
  Builtin_Encoder_Base64Encode,
  Builtin_Encoder_HexDecode,
  Builtin_Encoder_HexEncode,
  Builtin_Encoder_UrlDecode,
  Builtin_Encoder_UrlEncode,
  Builtin_Env_GetAll,
  Builtin_Env_GetArgv,
  Builtin_Env_GetEnvironmentVariable,
  Builtin_Env_GetXarg,
  Builtin_Env_Kiwi,
  Builtin_Env_OS,
  Builtin_Env_SetEnvironmentVariable,
  Builtin_Env_User,
  Builtin_Env_UserDomain,
  Builtin_FileIO_AppendText,
  Builtin_FileIO_ChangeDirectory,
  Builtin_FileIO_Combine,
  Builtin_FileIO_CopyFile,
  Builtin_FileIO_CopyR,
  Builtin_FileIO_CreateFile,
  Builtin_FileIO_DeleteFile,
  Builtin_FileIO_FileExists,
  Builtin_FileIO_FileName,
  Builtin_FileIO_FileSize,
  Builtin_FileIO_GetCurrentDirectory,
  Builtin_FileIO_GetFileAbsolutePath,
  Builtin_FileIO_GetFileExtension,
  Builtin_FileIO_GetFileInfo,
  Builtin_FileIO_GetFilePath,
  Builtin_FileIO_Glob,
  Builtin_FileIO_IsDirectory,
  Builtin_FileIO_ListDirectory,
  Builtin_FileIO_MakeDirectory,
  Builtin_FileIO_MakeDirectoryP,
  Builtin_FileIO_MoveFile,
  Builtin_FileIO_PathParts,
  Builtin_FileIO_ReadBytes,
  Builtin_FileIO_ReadFile,
  Builtin_FileIO_ReadLines,
  Builtin_FileIO_ReadSlice,
  Builtin_FileIO_RemoveDirectory,
  Builtin_FileIO_RemoveDirectoryF,
  Builtin_FileIO_TempDir,
  Builtin_FileIO_TempFile,
  Builtin_FileIO_WriteBytes,
  Builtin_FileIO_WriteLine,
  Builtin_FileIO_WriteSlice,
  Builtin_FileIO_WriteText,
  Builtin_Http_Delete,
  Builtin_Http_Download,
  Builtin_Http_Get,
  Builtin_Http_Head,
  Builtin_Http_Options,
  Builtin_Http_Patch,
  Builtin_Http_Post,
  Builtin_Http_Put,
  Builtin_Http_Timeout,
  Builtin_Ipc_Close,
  Builtin_Ipc_Connect,
  Builtin_Ipc_Create,
  Builtin_Ipc_Recv,
  Builtin_Ipc_Send,
  Builtin_Kiwi_ExecPath,
  Builtin_Kiwi_Main,
  Builtin_Kiwi_MemUsage,
  Builtin_Kiwi_Tokenize,
  Builtin_Kiwi_TypeOf,
  Builtin_List_All,
  Builtin_List_Each,
  Builtin_List_Filter,
  Builtin_List_GroupByField,
  Builtin_List_Map,
  Builtin_List_Max,
  Builtin_List_Min,
  Builtin_List_None,
  Builtin_List_Reduce,
  Builtin_List_Skip,
  Builtin_List_Sort,
  Builtin_List_Sum,
  Builtin_List_Take,
  Builtin_Math_Abs,
  Builtin_Math_Acos,
  Builtin_Math_Asin,
  Builtin_Math_Atan,
  Builtin_Math_Atan2,
  Builtin_Math_Cbrt,
  Builtin_Math_Ceil,
  Builtin_Math_CopySign,
  Builtin_Math_Cos,
  Builtin_Math_Cosh,
  Builtin_Math_Divisors,
  Builtin_Math_Epsilon,
  Builtin_Math_Erf,
  Builtin_Math_ErfC,
  Builtin_Math_Exp,
  Builtin_Math_ExpM1,
  Builtin_Math_FDim,
  Builtin_Math_Floor,
  Builtin_Math_FMax,
  Builtin_Math_FMin,
  Builtin_Math_Fmod,
  Builtin_Math_Hypot,
  Builtin_Math_IsFinite,
  Builtin_Math_IsInf,
  Builtin_Math_IsNaN,
  Builtin_Math_IsNormal,
  Builtin_Math_LGamma,
  Builtin_Math_Log,
  Builtin_Math_Log10,
  Builtin_Math_Log1P,
  Builtin_Math_Log2,
  Builtin_Math_NextAfter,
  Builtin_Math_Pow,
  Builtin_Math_Random,
  Builtin_Math_RandomSet,
  Builtin_Math_Remainder,
  Builtin_Math_RotateLeft,
  Builtin_Math_RotateRight,
  Builtin_Math_Round,
  Builtin_Math_Sin,
  Builtin_Math_Sinh,
  Builtin_Math_Sqrt,
  Builtin_Math_Tan,
  Builtin_Math_Tanh,
  Builtin_Math_TGamma,
  Builtin_Math_Trunc,
  Builtin_Reflector_CallStack,
  Builtin_Reflector_FrameFlags,
  Builtin_Reflector_GetFunc,
  Builtin_Reflector_ObjectContext,
  Builtin_Reflector_RetVal,
  Builtin_Reflector_State,
  Builtin_Serializer_Deserialize,
  Builtin_Serializer_Serialize,
  Builtin_Socket_Accept,
  Builtin_Socket_Close,
  Builtin_Socket_Receive,
  Builtin_Socket_Send,
  Builtin_Socket_TcpConnect,
  Builtin_Socket_TcpServer,
  Builtin_Stdin_Empty,
  Builtin_Stdin_Lines,
  Builtin_Stdin_Read,
  Builtin_Stdin_ReadBytes,
  Builtin_Stdin_ReadLine,
  Builtin_Sys_Exec,
  Builtin_Sys_ExecOut,
  Builtin_Sys_Kill,
  Builtin_Sys_Open,
  Builtin_Sys_Pid,
  Builtin_Sys_Pids,
  Builtin_Sys_Processes,
  Builtin_Sys_Running,
  Builtin_Sys_Spawn,
  Builtin_Sys_Wait,
  Builtin_Task_Await,
  Builtin_Task_Busy,
  Builtin_Task_List,
  Builtin_Task_Result,
  Builtin_Task_Sleep,
  Builtin_Task_Spawn,
  Builtin_Task_Status,
  Builtin_Time_AddDay,
  Builtin_Time_AddHour,
  Builtin_Time_AddMillisecond,
  Builtin_Time_AddMinute,
  Builtin_Time_AddMonth,
  Builtin_Time_AddSecond,
  Builtin_Time_AddYear,
  Builtin_Time_AMPM,
  Builtin_Time_Hour,
  Builtin_Time_IsDST,
  Builtin_Time_Millisecond,
  Builtin_Time_Minute,
  Builtin_Time_Month,
  Builtin_Time_MonthDay,
  Builtin_Time_Now,
  Builtin_Time_Second,
  Builtin_Time_Ticks,
  Builtin_Time_TicksToMilliseconds,
  Builtin_Time_WeekDay,
  Builtin_Time_Year,
  Builtin_Time_YearDay,
  Builtin_Tls_Accept,
  Builtin_Tls_Close,
  Builtin_Tls_Receive,
  Builtin_Tls_Send,
  Builtin_Tls_TcpConnect,
  Builtin_Tls_TcpServer,
  Builtin_Crypto_HmacMd5,
  Builtin_Crypto_HmacSha256,
  Builtin_Crypto_HmacSha512,
  Builtin_Crypto_Sha1,
  Builtin_Crypto_Sha384,
  Builtin_Crypto_Sha512,
  Builtin_HttpServer_Accept,
  Builtin_HttpServer_Create,
  Builtin_HttpServer_Respond,
  Builtin_HttpServer_Stop,
  Builtin_Mutex_Acquire,
  Builtin_Mutex_Create,
  Builtin_Mutex_Destroy,
  Builtin_Mutex_Release,
  Builtin_Mutex_TryAcquire,
  Builtin_Signal_Check,
  Builtin_Signal_Raise,
  Builtin_Signal_Reset,
  Builtin_Signal_Trap,
  Builtin_Smtp_Send,
  Builtin_Udp_Bind,
  Builtin_Udp_Close,
  Builtin_Udp_Create,
  Builtin_Udp_Recv,
  Builtin_Udp_Send,
  Builtin_Udp_SetTimeout,
  KW_Abstract,
  KW_As,
  KW_Break,
  KW_Case,
  KW_Catch,
  KW_Const,
  KW_Do,
  KW_Else,
  KW_ElseIf,
  KW_Emit,
  KW_End,
  KW_Enum,
  KW_EPrint,
  KW_EPrintLn,
  KW_Eval,
  KW_Exit,
  KW_Export,
  KW_False,
  KW_Finally,
  KW_For,
  KW_If,
  KW_Import,
  KW_In,
  KW_Include,
  KW_Interface,
  KW_Lambda,
  KW_Method,
  KW_Next,
  KW_Null,
  KW_Off,
  KW_On,
  KW_Once,
  KW_Override,
  KW_Package,
  KW_Pass,
  KW_Print,
  KW_PrintLn,
  KW_PrintXy,
  KW_Private,
  KW_Repeat,
  KW_Require,
  KW_Return,
  KW_Static,
  KW_Struct,
  KW_Super,
  KW_This,
  KW_StaticSelf,
  KW_Throw,
  KW_To,
  KW_True,
  KW_Try,
  KW_Var,
  KW_When,
  KW_While,
  KW_Yield,
  Ops_Add,
  Ops_AddAssign,
  Ops_And,
  Ops_AndAssign,
  Ops_Assign,
  Ops_BitwiseAnd,
  Ops_BitwiseAndAssign,
  Ops_BitwiseLeftShift,
  Ops_BitwiseLeftShiftAssign,
  Ops_BitwiseNot,
  Ops_BitwiseNotAssign,
  Ops_BitwiseOr,
  Ops_BitwiseOrAssign,
  Ops_BitwiseRightShift,
  Ops_BitwiseRightShiftAssign,
  Ops_BitwiseUnsignedRightShift,
  Ops_BitwiseUnsignedRightShiftAssign,
  Ops_BitwiseXor,
  Ops_BitwiseXorAssign,
  Ops_Divide,
  Ops_DivideAssign,
  Ops_Equal,
  Ops_Exponent,
  Ops_ExponentAssign,
  Ops_GreaterThan,
  Ops_GreaterThanOrEqual,
  Ops_LessThan,
  Ops_LessThanOrEqual,
  Ops_ModuloAssign,
  Ops_Modulus,
  Ops_Multiply,
  Ops_MultiplyAssign,
  Ops_Not,
  Ops_NotEqual,
  Ops_NullCoalesce,
  Ops_Or,
  Ops_OrAssign,
  Ops_Question,
  Ops_Subtract,
  Ops_SubtractAssign,
  Ops_Unpack,
  Types_Any,
  Types_Boolean,
  Types_Bytes,
  Types_Date,
  Types_Float,
  Types_Generator,
  Types_Hashmap,
  Types_Integer,
  Types_Lambda,
  Types_List,
  Types_None,
  Types_Object,
  Types_Pointer,
  Types_String
}