class Script {
  ...
  public:
    ...
    Script(std::string name) { scriptName = name; }
    ...
    void addMark(std::string mark) { marks.push_back(mark); }
    ...
    int markSize() { return (int)marks.size(); }

    int size() { return get().size(); }

    std::string name() { return scriptName; }
};